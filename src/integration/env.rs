...
pub(crate) const CODELY_CONFIG_DIR_ENV_VAR: &str = "CODELY_CONFIG_DIR";
...
pub(crate) fn codely_dir() -> io::Result<PathBuf> {
    // CODELY_CONFIG_DIR is a herdr-level override only (primarily a test
    // seam); codely keeps its settings and hooks in ~/.codely-cli.
    if let Some(value) =
        std::env::var_os(CODELY_CONFIG_DIR_ENV_VAR).filter(|value| !value.is_empty())
    {
        return expand_tilde_path(PathBuf::from(value));
    }
    Ok(home_dir()?.join(".codely-cli"))
}
