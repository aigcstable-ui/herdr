...
use super::targets::{
    install_antigravity_cli, install_claude, install_codely, install_codex, install_copilot,
    ...
    install_qodercli, install_qwen, uninstall_antigravity_cli, uninstall_claude, uninstall_codely,
...
fn install_target_inner(target: crate::api::schema::IntegrationTarget) -> io::Result<Vec<String>> {
    ...
    let mut messages = match target {
        ...
        crate::api::schema::IntegrationTarget::Codely => {
            let installed = install_codely()?;
            vec![
                format!(
                    "installed codely integration hook to {}",
                    installed.hook_path.display()
                ),
                format!(
                    "registered codely hooks in {}",
                    installed.settings_path.display()
                ),
            ]
        }
...
pub(crate) fn uninstall_target(
    ...
    let messages = match target {
        ...
        crate::api::schema::IntegrationTarget::Codely => {
            let result = uninstall_codely()?;
            let mut messages = Vec::new();
            if result.removed_hook_file {
                messages.push(format!(
                    "removed codely hook at {}",
                    result.hook_path.display()
                ));
            } else {
                messages.push(format!(
                    "no codely hook found at {}",
                    result.hook_path.display()
                ));
            }
            if result.updated_settings {
                messages.push(format!(
                    "removed codely hook entries from {}",
                    result.settings_path.display()
                ));
            }
            messages
