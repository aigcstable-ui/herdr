...
fn parse_integration_target(
    ...
    let parsed = match target {
        ...
        "codely" => IntegrationTarget::Codely,
        _ => {
            eprintln!("unknown integration target: {target}");
            eprintln!(
                "currently supported: pi, omp, claude, codex, copilot, devin, droid, kimi, opencode, kilo, hermes, qodercli, qwen, cursor, mastracode, antigravity-cli, grok, codely"
            );
            return Ok(None);
        }
    };
