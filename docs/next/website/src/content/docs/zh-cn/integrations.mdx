---
title: 集成
description: 为 Pi、OMP、Claude Code、Codex、GitHub Copilot CLI、Devin CLI、Droid、Kimi Code CLI、OpenCode、Kilo Code CLI、Hermes Agent、Qoder CLI、Qwen Code、Letta Code、Cursor Agent CLI、MastraCode、Antigravity CLI、Grok CLI 和 Codely CLI 安装 Herdr 集成。
---
...
| 集成类型 | 智能体 | 效果 |
| --- | --- | --- |
| 生命周期权威 | Pi、OMP、Kimi Code CLI、OpenCode、Kilo Code CLI、MastraCode、Codely CLI | 已安装且在为该窗格主动上报时,由钩子或插件事件决定 `idle`、`working` 和 `blocked`。对同一个生命周期权威,Herdr 不再使用屏幕清单兜底。 |
...
一些集成会上报智能体的原生会话引用。除非被 `[session] resume_agents_on_restore = false` 禁用,Herdr 会在服务器重启后使用官方会话引用恢复 Claude Code、Codex、Devin CLI、Droid、Kimi Code CLI、Qoder CLI、Qwen Code、Letta Code、Cursor Agent CLI、Grok CLI、GitHub Copilot CLI、Pi、OMP、Hermes Agent、OpenCode、Kilo Code CLI、MastraCode、Antigravity CLI 和 Codely CLI 的窗格。

原生会话恢复需要最新的 Herdr 集成: Pi 集成版本 `2`、OMP 版本 `3`、Claude Code 版本 `6`、Codex 版本 `5`、GitHub Copilot CLI 版本 `2`、Devin CLI 版本 `2`、Droid 版本 `2`、Kimi Code CLI 版本 `3`、Qoder CLI 版本 `2`、Qwen Code 版本 `1`、Letta Code 版本 `1`、Cursor Agent CLI 版本 `1`、Grok CLI 版本 `2`、OpenCode 版本 `5`、Kilo Code CLI 版本 `1`、Hermes Agent 版本 `5`、MastraCode 版本 `1`、Antigravity CLI 版本 `1`、Codely CLI 版本 `1`。用 `herdr integration status` 查看已安装版本。
...
该钩子向 Herdr 上报 Codely CLI 的生命周期状态与会话身份,用于权威的 `idle`、`working`、`blocked` 状态和原生恢复。钩子已安装且持续上报时,它是状态权威;否则 Herdr 回退到 Codely CLI 的屏幕清单检测。

Herdr 默认使用 `~/.codely-cli`。Codely 配置目录必须已经存在。安装会写入 `hooks/herdr-agent-state.js`,并在 `settings.json` 中注册 Herdr 管理的条目。Codely 的钩子系统默认关闭,因此安装也会启用 `hooks.enabled`。卸载会移除 Herdr 条目并删除钩子脚本,但保留 `hooks.enabled` 不变,以免影响你自己的钩子。

Codely 会在每个项目首次遇到新钩子时要求信任;首次运行时请通过 Codely 的 `/hooks` 信任流程确认 Herdr 钩子,或设置 `CODELY_TRUST_ALL_HOOKS=1` 自动确认。

Herdr 可以用 `codely --resume-session <id>` 恢复已存储的 Codely 会话。
