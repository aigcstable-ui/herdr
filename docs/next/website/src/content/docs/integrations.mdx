---
title: Integrations
description: Install Herdr integrations for Pi, OMP, Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Kimi Code CLI, OpenCode, Kilo Code CLI, Hermes Agent, Qoder CLI, Qwen Code, Letta Code, Cursor Agent CLI, MastraCode, Antigravity CLI, Grok CLI, and Codely CLI.
---
...
herdr integration install codely
```
...
herdr integration uninstall codely
```
...
| --- | --- | --- |
| Lifecycle authority | Pi, OMP, Kimi Code CLI, OpenCode, Kilo Code CLI, MastraCode, Codely CLI | When installed and actively reporting for the pane, hook or plugin events author `idle`, `working`, and `blocked`. Herdr does not also use screen manifest fallback for that same lifecycle authority. |
...
Install the Codely CLI hook:

```bash
herdr integration install codely
```

The hook reports Codely CLI lifecycle state and session identity to Herdr for authoritative `idle`, `working`, and `blocked` status and native restore. While the hook is installed and actively reporting, it is the state authority; otherwise Herdr falls back to Codely CLI screen manifest detection.

Herdr uses `~/.codely-cli` by default. The Codely config directory must already exist. Install writes `hooks/herdr-agent-state.js` and registers Herdr-managed entries in `settings.json`. Codely keeps its hook system off by default, so install also enables `hooks.enabled`. Uninstall removes the Herdr entries and deletes the hook script, and leaves `hooks.enabled` unchanged for your own hooks.

Codely asks once per project to trust a newly seen hook; confirm the Herdr hook through Codely's `/hooks` trust flow the first time it runs, or set `CODELY_TRUST_ALL_HOOKS=1` to confirm automatically.

Herdr resumes stored Codely sessions with `codely --resume-session <id>`.
