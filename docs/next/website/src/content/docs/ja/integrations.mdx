---
title: インテグレーション
description: Pi、OMP、Claude Code、Codex、GitHub Copilot CLI、Devin CLI、Droid、Kimi Code CLI、OpenCode、Kilo Code CLI、Hermes Agent、Qoder CLI、Qwen Code、Letta Code、Cursor Agent CLI、MastraCode、Antigravity CLI、Grok CLI、Codely CLI 向けの Herdr インテグレーションをインストールします。
---
...
| --- | --- | --- |
| ライフサイクル権威 | Pi、OMP、Kimi Code CLI、OpenCode、Kilo Code CLI、MastraCode、Codely CLI | インストールされ、そのペインについて能動的に報告している間は、フックまたはプラグインのイベントが `idle`、`working`、`blocked` を決定します。同じライフサイクル権威に対して、Herdr はスクリーンマニフェストのフォールバックを併用しません。 |
...
Codely CLI フックをインストールします:

```bash
herdr integration install codely
```

このフックは、Codely CLI のライフサイクル状態とセッション識別を Herdr に報告し、権威ある `idle`・`working`・`blocked` の状態とネイティブ復元を実現します。フックがインストールされ、継続的に報告している間はフックが状態の権威となり、そうでない場合は Herdr が Codely CLI のスクリーンマニフェスト検出にフォールバックします。

Herdr はデフォルトで `~/.codely-cli` を使います。Codely の設定ディレクトリはあらかじめ存在している必要があります。インストールは `hooks/herdr-agent-state.js` を書き込み、`settings.json` に Herdr 管理のエントリを登録します。Codely はフックシステムを既定でオフにしているため、インストールは `hooks.enabled` も有効にします。アンインストールは Herdr のエントリを削除しフックスクリプトを削除しますが、ユーザー自身のフックのために `hooks.enabled` は変更しません。

Codely は、初めて見るフックについてプロジェクトごとに 1 回信頼を求めます。初回実行時に Codely の `/hooks` 信頼フローで Herdr フックを承認するか、`CODELY_TRUST_ALL_HOOKS=1` を設定して自動的に承認してください。

Herdr は保存された Codely セッションを `codely --resume-session <id>` で resume できます。
